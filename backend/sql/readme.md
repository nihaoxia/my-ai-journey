...
CREATE TABLE `user_like_post` (
  ...
  CONSTRAINT `user_like_post_ibfk_2` FOREIGN KEY (`postId`) REFERENCES `post` (`postId`) ON DELETE CASCADE ON UPDATE CASCADE
)
```
索引的认识， 聚例子 
不用单独建立 userId key, 因为 联合主键 userId postId, 已经覆盖了，
浪费空间
...
CREATE TABLE `file` (
  ...
  CONSTRAINT `file_ibfk_2` FOREIGN KEY (`postId`) REFERENCES `post` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
)
```
