...
## Message 
SystemMessage  设置AI是谁 可以干什么 有什么能力 以及一些回答行为
               的规范等
HumanMessage   用户的问题
AIMessage      AI 的回复消息
ToolMessage    工具调用的结果返回Tool id
原生的openai 返回工具调用 additional_kwargs -> tools -> 每个tool langchain invoke 原样输出上面的，同时还会细心地准备tools 加到后面
llm工程开发的便捷性、可读性 帮助
...
- src 开发代码目录
   - promise 特性
   async 函数 就是promise 实例， return resolve 并且return 的结果就是
   ...
   - langchain 
      ...
      langchain 工具流 ChatOpenAI -> tools -> bindTools -> invoke
      llm 工作流编排框架
    ...
    - Agent 工作流程
      - llm能力边界
        stateless + 不能直接干活
    - 不停地维护messages 数组
    - llm不能直接生成，直接返回 带tool 的消息
    - tool 执行 ToolMessage tool_id 加入
    - 最简单的loop 有工具调用
       没有 拿着所有的 messages 去最后一次调用llm 完成任务 拿到结果
- Promise 升级
