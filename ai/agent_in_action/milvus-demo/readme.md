...
## Milvus
文档向量化放到向量数据库 每次查询根据向量化的query 去数据库做相似度匹配 查出相关文档放到peompt 里给大模型 大模型来生成回答

- 从内存到向量数据库
Milvus 是一款开源的向量数据库，转为处理海量高维向量数据而设计。
AI Agent 产品都会使用Milvus 这样的vector store

像 web 应用会把数据存在mysql里, sqlite ,psql 基于对数据的增删改查 实现各种业务功能。CRUD
